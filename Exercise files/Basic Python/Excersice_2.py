...
"""
def average(rlis):
    average = sum(rlis)/len(rlis)
    print("Average is",average)
#%%
def average(nlis):
    average = sum(nlis)/len(nlis)
    print("Average is",average)    
    

#%%
    
numlis = [1,2,6,4,8,3,4,2,2,3,7,5,9]
def average(numlis):
    sum_=0
    for i in range(0,len(numlis)):
        sum_=sum_+numlis[i]
        print(numlis[i],end=" ")
    Avg = int (sum_/len(numlis))
    print()
    print(Avg)    
...
"""
#%%
newEngland = [["Massachusetts",1],["Connecticut",2],
              ["Maine",2],["New Hampshire",2],
              ["Rhode Island",3],["Vermont",5]]
...
"""
#%%
nlis = [65, 44, 3, 56, 48, 74, 7, 97, 95, 42]  # test on this list
...
def average(nlis):
   sum_ = 0
   for item in range(0,len(nlis)):
       sum_= sum_ + nlis[item]
       print(nlis[item],end=" ")
   average = (sum_)/len(nlis)
   print()
